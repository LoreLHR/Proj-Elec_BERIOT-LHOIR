function varargout = APLQ(varargin)

format long e;
% APLQ M-file for APLQ.fig
%      APLQ, by itself, creates a new APLQ or raises the existing
%      singleton*.
%
%      H = APLQ returns the handle to a new APLQ or the handle to
%      the existing singleton*.
%
%      APLQ('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in APLQ.M with the given input arguments.
%
%      APLQ('Property','Value',...) creates a new APLQ or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before APLQ_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to APLQ_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help APLQ

% Last Modified by GUIDE v2.5 13-Apr-2006 15:05:29

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @APLQ_OpeningFcn, ...
                   'gui_OutputFcn',  @APLQ_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before APLQ is made visible.
function APLQ_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to APLQ (see VARARGIN)

% Choose default command line output for APLQ
...
% UIWAIT makes APLQ wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = APLQ_OutputFcn(hObject, eventdata, handles)
...
if ((f_p~=0)&&(q_p~=0)&&(K~=0))
    ...
    w_p=2*pi*f_p;
    figure;
    freqs(K*[1 -w_p/q_p w_p^2],[1 w_p/q_p w_p^2]);
else
