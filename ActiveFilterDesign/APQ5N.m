function varargout = APQ5N(varargin)

format long e;
% APQ5N M-file for APQ5N.fig
%      APQ5N, by itself, creates a new APQ5N or raises the existing
%      singleton*.
%
%      H = APQ5N returns the handle to a new APQ5N or the handle to
%      the existing singleton*.
%
%      APQ5N('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in APQ5N.M with the given input arguments.
%
%      APQ5N('Property','Value',...) creates a new APQ5N or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before APQ5N_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to APQ5N_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help APQ5N

% Last Modified by GUIDE v2.5 13-Apr-2006 23:44:28

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @APQ5N_OpeningFcn, ...
                   'gui_OutputFcn',  @APQ5N_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before APQ5N is made visible.
function APQ5N_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to APQ5N (see VARARGIN)
movegui(hObject,'center')  % To display application in the center of screen


% Choose default command line output for APQ5N
...
% UIWAIT makes APQ5N wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = APQ5N_OutputFcn(hObject, eventdata, handles)
...
if ((f_p~=0)&&(q_p~=0))
    ...
    H=1-((C1+C2)*(C2+C3)*4*(q_p^2))/(C1*C2);
    if H<0
        set(handles.comments, 'String', 'No plot ');
    else
...
%copy-paste
a=textread('APQ5N.cir','%s','delimiter', '');
