function varargout = BRHQ(varargin)

format long e;
% BRHQ M-file for BRHQ.fig
%      BRHQ, by itself, creates a new BRHQ or raises the existing
%      singleton*.
%
%      H = BRHQ returns the handle to a new BRHQ or the handle to
%      the existing singleton*.
%
%      BRHQ('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in BRHQ.M with the given input arguments.
%
%      BRHQ('Property','Value',...) creates a new BRHQ or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before BRHQ_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to BRHQ_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help BRHQ

% Last Modified by GUIDE v2.5 13-Apr-2006 22:02:57

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @BRHQ_OpeningFcn, ...
                   'gui_OutputFcn',  @BRHQ_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before BRHQ is made visible.
function BRHQ_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to BRHQ (see VARARGIN)
movegui(hObject,'center')  % To display application in the center of screen

% Choose default command line output for BRHQ
...
% UIWAIT makes BRHQ wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = BRHQ_OutputFcn(hObject, eventdata, handles)
...
if ((f_p~=0)&&(q_p~=0))
    ...
    w_p=2*pi*f_p;
    figure;
    freqs([1 0 w_p^2],[1 w_p/q_p w_p^2]);
else
...
%copy-paste
a=textread('BRHQ.cir','%s','delimiter', '');
