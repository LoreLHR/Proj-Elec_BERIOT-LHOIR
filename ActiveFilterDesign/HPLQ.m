function varargout = HPLQ(varargin)

format long e;
% HPLQ M-file for HPLQ.fig
%      HPLQ, by itself, creates a new HPLQ or raises the existing
%      singleton*.
%
%      H = HPLQ returns the handle to a new HPLQ or the handle to
%      the existing singleton*.
%
%      HPLQ('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in HPLQ.M with the given input arguments.
%
%      HPLQ('Property','Value',...) creates a new HPLQ or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before HPLQ_OpeningFunction gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to HPLQ_OpeningFcn via varargin.
%
...
% Edit the above text to modify the response to help HPLQ

% Last Modified by GUIDE v2.5 13-Apr-2006 14:47:35

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @HPLQ_OpeningFcn, ...
                   'gui_OutputFcn',  @HPLQ_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before HPLQ is made visible.
function HPLQ_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to HPLQ (see VARARGIN)

% Choose default command line output for HPLQ
...
% UIWAIT makes HPLQ wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = HPLQ_OutputFcn(hObject, eventdata, handles)
...
C1=(C11+C12);
P=((q_p^2)*(2+(C3/C1)+(C1/C3)));
R2=(1/(2*pi*f_p*sqrt(P*C1*C3)));
R4=(P*R2);
GSP=(q_p*sqrt(P*C3/C1));
K=(C11/C1);
...
%copy-paste
a=textread('HPLQ.cir','%s','delimiter', '');
